...
export default function Footer({ locale }: FooterProps) {
  ...
  const footerTranslations: Record<string, any> = {
    ...
    pt: {
      ...
      aboutText: "A MH India Trips desenha viagens privadas de luxo e passeios de monumentos históricos em todo o subcontinente indiano. Fundada em 2010 com paixão pela qualidade.",
  ...
  return (
    <footer className="bg-royal text-white/60 font-sans relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-16 lg:gap-12">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="space-y-4">
              ...
              />
              <p className="text-[8px] uppercase tracking-[0.3em] text-gold font-black">
                {labels.tagline}
              </p>
            </div>
            <p className="text-xs leading-relaxed text-white/50 max-w-sm font-light">
              {labels.aboutText}
            </p>
            
            {/* Social Grid */}
            <div className="flex items-center gap-3.5 pt-2">
              <a href="https://www.facebook.com/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gold hover:border-gold hover:text-royal transition-all duration-300 text-white/50" aria-label="Facebook">
                <svg className="w-4 h-4 fill-currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href="https://www.instagram.com/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gold hover:border-gold hover:text-royal transition-all duration-300 text-white/50" aria-label="Instagram">
              ...
              <a href="https://wa.me/919876543210" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gold hover:border-gold hover:text-royal transition-all duration-300 text-white/50" aria-label="WhatsApp">
              ...
              <a href="https://www.youtube.com/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gold hover:border-gold hover:text-royal transition-all duration-300 text-white/50" aria-label="YouTube">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h4 className="text-[10px] uppercase tracking-[0.2em] text-gold font-bold pb-2.5 border-b border-white/10">
              {labels.quickLinks}
            </h4>
              ...
              {quickLinks.map((link, i) => (
                <li key={i}>
                  <Link
                    ...
                    className="hover:text-gold flex items-center gap-2 transition-colors duration-200"
                  >
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubscribe} className="flex bg-white/5 border border-white/10 rounded-full p-1.5 focus-within:border-gold/30 transition-colors">
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder={labels.newsletterPlaceholder}
                  className="bg-transparent text-xs pl-4 outline-none w-full text-white placeholder-white/20 font-light"
                  required
                />
                <button type="submit" className="bg-gold hover:bg-gold-light text-royal rounded-full p-2.5 transition-transform duration-200 hover:scale-105 shrink-0" aria-label="Subscribe">
            ...
            <div className="space-y-2 pt-4 border-t border-white/10 text-xs">
              <a href="tel:+919876543210" className="flex items-center gap-2 text-white/50 hover:text-gold transition-colors">
                <Phone className="w-3.5 h-3.5 text-gold" />
                <span>+91 98765 43210</span>
              </a>
              <a href="mailto:hello@mhindiatrips.com" className="flex items-center gap-2 text-white/50 hover:text-gold transition-colors">
        ...
        <div className="border-t border-white/10 mt-16 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-white/35 font-light">
