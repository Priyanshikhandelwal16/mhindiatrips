...
export default function PopupInquiryForm({ locale, contactDetails }: PopupInquiryFormProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("open-inquiry-popup", handleOpenTrigger);
    return () => {
      window.removeEventListener("open-inquiry-popup", handleOpenTrigger);
    };
  }, []);
  ...
  return (
    <div className="fixed inset-0 z-[1000000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 text-xs">
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-3">
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 bg-royal hover:bg-gold hover:text-royal text-white font-bold uppercase tracking-wider py-3.5 px-4 rounded-2xl shadow-lg transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed text-[11px] leading-tight min-h-[46px]"
                  >
                    <Send className="w-4 h-4 shrink-0 text-gold" />
                    <span className="text-center">{loading ? text.submitting : text.submit}</span>
                  </button>
